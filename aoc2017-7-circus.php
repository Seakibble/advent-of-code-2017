 ...
 *
 * This puzzle was called 'Recursive Circus'. So naturally I did it *without* using recursion. Oh the agony.
 */
 ...
 * PART 1
 */
function circus1($input)
{
    $str = $input;
    $array = explode("\n", $str);

    $towers = [];
    foreach ($array as $tower) {
        $name = explode(' (', $tower);
        $towers[$name[0]] = $tower;
    }
    ...
    foreach ($towers as $name => $tower) {
        if (preg_match("/ $name/", $str)) {
            unset($towers[$name]);
            foreach ($towers as $otherName => $otherTower) {
                $towers[$otherName] = str_replace($name, "{ $tower }", $otherTower);
                if ($otherTower !== $towers[$otherName]) {
                    break;
                }
            }
        }
    ...
    echo 'The root program is ' . $matches[1] . ".\n";
}
...
function circus2($input)
{
    ...
    foreach ($programs as $name) {
        $thisTower = $towers[$name];

        if (preg_match("/ $name/", $str)) {
            unset($towers[$name]);
            foreach ($towers as $otherName => $otherTower) {
                $towers[$otherName] = str_replace($name, "{ $thisTower }", $otherTower);
                if ($otherTower !== $towers[$otherName]) {
                    break;
                }
            }
    ...
    foreach ($cmds as $cmd) {
        switch ($cmd) {
            ...
            case '}':
                ...
                $foundTarget = false;
                foreach ($weights[$level] as $weight) {
                    if ($sampleWeight == 0) {
                        $sampleWeight = $weight;
                    } elseif ($sampleWeight != $weight && count($weights[$level]) > 2) {
                        $foundTarget = true;
                    }
                    $total += $weight;
                }
                ...
                if ($foundTarget) {
                    $oddOneOut = [];
                    foreach ($weights[$level] as $name => $weight) {
                        $oddOneOut[$weight][$name] = $lookup[$name];
                    }

                    $keys = array_keys($oddOneOut);
                    if (count($oddOneOut[$keys[0]]) > 1) {
                        $a = $keys[0];
                        $b = $keys[1];
                    } else {
                        $a = $keys[1];
                    ...
                    $difference = $a - $b;
                    $currentWeight = current($oddOneOut[$b]);
                    $name = array_keys($oddOneOut[$b]);

                    echo "The imbalanced program is $name[0] and its weight should be " . ($currentWeight + $difference) . ".\n";
                    return;
                }
                ...
                $weights[$level][key($weights[$level])] += $total;
                break;

            default:
                $program = explode('-', $cmd);
                $lookup[$program[0]] = (int)$program[1];
                $weights[$level][$program[0]] = (int)$program[1];
        }
    }
}
