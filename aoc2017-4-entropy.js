 ...
 */
{
var data = "vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh\n" +
 ...
 * PART 1
 */
function entropy1(input) {
    var rows = String(input).split('\n');

    var phrases = [];
    rows.forEach(function (row) {
        var splitRow = String(row).split(' ');
        var parsedRow = [];
        splitRow.forEach(function (cell) {
            parsedRow.push(cell);
        }, splitRow);

        phrases.push(parsedRow);
    }, rows);

    var validPhrases = 0;
    for (var i = 0; i < phrases.length; i++) {
        if (isValid(phrases[i])) {
            validPhrases++;
        }
    }
    console.log("Total valid passphrases: " + validPhrases);
...
function entropy2(input) {
    ...
    function isAnagram(A, B) {
        ...
        for (var i = 0; i < b.length; i++) {
            if (aLetters[b[i]] === undefined) {
                return false;
            }
            if (bLetters[b[i]] !== undefined) {
                bLetters[b[i]]++;
            } else {
                bLetters[b[i]] = 1;
            }
        ...
        for (var letter in aLetters) {
            if (!bLetters.hasOwnProperty(letter) || bLetters[letter] !== aLetters[letter]) {
                return false;
            }
        }
        return true;
