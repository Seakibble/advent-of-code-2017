...
function registers1($input)
{
    ...
    foreach ($commands as $command) {
        ...
        $modElement = $command[0];
        $modOperator = $command[1];
        $modAmount = $command[2];

        $ifElement = $command[4];
        $ifOperator = $command[5];
        $ifAmount = $command[6];

        if (!isset($registers[$modElement])) {
            $registers[$modElement] = 0;
        }

        if (!isset($registers[$ifElement])) {
            $registers[$ifElement] = 0;
        }

        $condition = false;
        eval('$condition' . " = ($registers[$ifElement] $ifOperator $ifAmount);");
        if ($condition && $modOperator === 'inc') {
            $registers[$modElement] += $modAmount;
        } elseif ($condition) {
            $registers[$modElement] -= $modAmount;
        }
    }
    ...
    reset($registers);
    $finalName = key($registers);
    $finalValue = $registers[$finalName];
    ...
    echo "The highest register is '$finalName', with a value of $finalValue.";
}
 ...
 * PART 2
 */
function registers2($input)
{
    $commands = explode("\n", $input);
    $registers = [];

    $totalHighestName = null;
    ...
    foreach ($commands as $command) {
        ...
        if ($totalHighestValue == null || $registers[$modElement] > $totalHighestValue) {
            $totalHighestValue = $registers[$modElement];
            $totalHighestName = $modElement;
        }
    }
    ...
    echo "The final highest register is '$finalName', with a value of $finalValue.\n";
    echo "The highest register during this process was '$totalHighestName', with a value of $totalHighestValue.";
}
